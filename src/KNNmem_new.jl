...
mutable struct KNNmemory{T <: Real}
    ...
    V::Vector{Int} # values in the memory (labels)
    A::Vector{Int} # age of a given key-value pair
    k::Int # number of neighbors used in kNN
    α # parameter setting the required distance between the nearest positive and negative sample in the kNN
    ...
    # Arguments
    - `memorySize::Int`: number of keys that can be stored in the memoryUpdate!
    - `keySize::Int`: length of a key
    - `k::Int`: number of k nearest neighbors to look for
    - `labelCount::Int`: number of labels that are in the dataset (used for the random initialization)
    ...
    """
    function KNNmemory{T}(memorySize::Int, keySize::Int, k::Int, labelCount::Int, encoder = x -> normalizecolumns(x), α::Real = 0.1) where T
...
`kLargestIDs::Vector{<:Int}` contains k indices leading to the k most similar keys in the memorySize.
"""
function findNearestPositiveAndNegative(memory::KNNmemory, kLargestIDs::Vector{<:Int}, v::Int)
...
function memoryLoss(memory::KNNmemory{T}, normalizedQuery::AbstractArray, nearestPositiveID::Int, nearestNegativeID::Int) where {T}    loss = max(dot(normalizedQuery, memory.encoder(memory.M[nearestNegativeID, :])) - dot(normalizedQuery, memory.encoder(memory.M[nearestPositiveID, :])) + memory.α, 0)
end
...
"""
function memoryUpdate!(memory::KNNmemory{T}, q::Vector, v::Int, nearestNeighbourID::Int) where {T}
...
function vmf_mix_lkh(x, μs, κ, μlength::Int)
...
"""
trainQuery!(memory::KNNmemory{T}, q::AbstractArray{T, N} where N, v::Int) where {T} = trainQuery!(memory, q, [v])

function trainQuery!(memory::KNNmemory{T}, q::AbstractArray{Tt, N} where {Tt, N}, v::Vector{<:Int}) where {T}
    ...
    nearestNeighbourIDs = zeros(Int, batchSize)
